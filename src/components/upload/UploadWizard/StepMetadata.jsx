...
import { buildIdentifier, validateMetadata, validatePdfMagic } from '../../../lib/metadata'
...
export default function StepMetadata() {
  const { metadata, file, setMetadata, setFile, setIdentifier, setStep } = useWizardStore()
  ...
  const identifierPreview = (metadata.institution?.qid && metadata.year && metadata.examType)
    ? buildIdentifier({
  ...
  const handleFileSelect = useCallback(async (selectedFile) => {
    ...
    if (selectedFile.size > MAX_FILE_SIZE) {
      setFileError('File size must be under 50MB. Current size: ' + (selectedFile.size / 1024 / 1024).toFixed(1) + 'MB.')
      return
    }
  ...
  const inputClasses = 'w-full rounded-lg border border-pyqp-border bg-pyqp-card px-3 py-2 text-sm text-pyqp-text placeholder:text-pyqp-muted focus:outline-none focus:ring-2 focus:ring-pyqp-accent/40 focus:border-pyqp-accent transition-colors'
  const labelClasses = 'block text-sm font-medium text-pyqp-text mb-1.5'
  const selectClasses = 'w-full rounded-lg border border-pyqp-border bg-pyqp-card px-3 py-2 text-sm text-pyqp-text focus:outline-none focus:ring-2 focus:ring-pyqp-accent/40 focus:border-pyqp-accent transition-colors appearance-none cursor-pointer'

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="border-b border-pyqp-border pb-4 mb-6">
        <h2 className="font-heading text-xl font-bold text-pyqp-text">Upload a Question Paper</h2>
        <p className="mt-1 text-sm text-pyqp-muted">Fill in the paper details below. Fields marked with * are required.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2">
          ...
          />
          <p className="mt-1 text-xs text-pyqp-muted">Search powered by Wikidata. Start typing to find your university.</p>
        </div>

        {/* Program */}
        <div>
          <label htmlFor="meta-program" className={labelClasses}>Program / Degree</label>
          <input
            id="meta-program"
            type="text"
        ...
        {/* Course Name */}
        <div>
          <label htmlFor="meta-course-name" className={labelClasses}>Course Name</label>
          <input
            id="meta-course-name"
            type="text"
        ...
        <div>
          <label htmlFor="meta-course-code" className={labelClasses}>
            Course Code <span className="text-pyqp-muted font-normal">(optional)</span>
          </label>
          <input
            id="meta-course-code"
            type="text"
        ...
        {/* Language */}
        <div>
          <label htmlFor="meta-language" className={labelClasses}>Language</label>
          <div className="relative">
            ...
            <svg className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-pyqp-muted pointer-events-none" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
        ...
        {/* Month */}
        <div>
          <label htmlFor="meta-month" className={labelClasses}>Month / Session</label>
          <div className="relative">
            <select
              id="meta-month"
        ...
        {/* Semester */}
        <div>
          <label htmlFor="meta-semester" className={labelClasses}>Semester</label>
          <div className="relative">
            <select
              id="meta-semester"
          ...
          <fieldset>
            ...
            <div className="flex flex-wrap gap-4 mt-1">
              {EXAM_TYPES.map((type) => (
                <label
                  key={type.value}
                  className={'inline-flex items-center gap-2 cursor-pointer rounded-lg border px-4 py-2 text-sm transition-colors ' + (
        ...
        <div className="md:col-span-2">
          ...
          {!file ? (
            ...
            >
              <svg className="mx-auto h-10 w-10 text-pyqp-muted mb-3" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m6.75 12l-3-3m0 0l-3 3m3-3v6m-1.5-15H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
              <p className="text-sm font-medium text-pyqp-text">Drop PDF here or click to browse</p>
              <p className="mt-1 text-xs text-pyqp-muted">Max 50MB &middot; PDF only</p>
              <input
                ref={fileInputRef}
                type="file"
                accept=".pdf"
            ...
            <div className="rounded-lg border border-pyqp-border bg-pyqp-card px-4 py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-pyqp-text truncate">{file.name}</p>
                  <p className="text-xs text-pyqp-muted">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                </div>
              </div>
              <button
                type="button"
        ...
        <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3 space-y-1">
          {errors.map((err, i) => (
            <p key={i} className="text-sm text-red-700 flex items-start gap-2">
              <svg className="h-4 w-4 mt-0.5 shrink-0 text-red-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
              </svg>
              <span>{err}</span>
            </p>
